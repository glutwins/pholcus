...
func (self *Context) AddQueue(req *request.Request) *Context {
	err := req.
		SetEnableCookie(self.spider.EnableCookie).
		Prepare()
...
// ruleName为空时默认当前规则。
func (self *Context) Output(item map[string]interface{}, ruleName ...string) {
	...
	self.Lock()
	self.items = append(self.items, data.GetDataCell(_ruleName, item, self.GetUrl(), self.GetReferer(), time.Now().Format("2006-01-02 15:04:05")))
	self.Unlock()
}
...
func (self *Context) getRule(ruleName ...string) (name string, rule *Rule, found bool) {
	...
	rule, found = self.spider.RuleTree.Trunk[name]
	return
}
